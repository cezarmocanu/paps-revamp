...
export const CategoriesExpandableList = () => {

  const renderSubcategories = (categ: any) => {
    const hasSubcategories = categ.subcategories.length > 0
    return hasSubcategories ? (<AccordionPanel key={categ.id}>
    <AccordionTitle>{categ.name}</AccordionTitle>
    {categ.subcategories.length > 0 ? (<AccordionContent>
